...
public static class AssetStoreBatchMode
{
    ...
    public static void UploadAssetStorePackage(params string[] args)
    {
        ...
        UploadAssetStorePackage(username, password, packageName, new []{rootPath}, mainAssets.ToArray(), loginTimeout, metadataTimeout, uploadTimeout, skipProjectSettings);
    }
    /// <summary>
    ...
    /// <param name="rootPaths">The root path of the package (relative to Application.dataPath). If null, use the project Assets folder.</param>
    ...
    public static void UploadAssetStorePackage(string username, string password, string packageName, string[] rootPaths = null, string[] mainAssets = null, int loginTimeout = 90, int metadataTimeout = 600, int uploadTimeout = 36000, bool skipProjectSettings = false)
    {
        ...
        {
            string projectFolder = Path.Combine(Application.dataPath, rootPath ?? string.Empty);
        ...
        // Set root asset path
        List<string> localRootPaths = new List<string>();
        foreach (string projectFolder in projectFolders)
        {
            string localRootPath = SetRootPath(package, projectFolder);
            localRootPaths.Add(localRootPath);
        ...
        var draftAssetsPath = GetDraftAssetsPath(localRootPaths[0]);

        Export(package, localRootPaths, draftAssetsPath);
    ...
    static void Export(Package package, List<string> localRootPaths, string toPath)
    {
        File.Delete(toPath);

        List<string> paths = new List<string>();

        foreach (string localRootPath in localRootPaths)
        {
        ...
        AssetDatabase.ExportPackage(paths.ToArray(), toPath);
    }
