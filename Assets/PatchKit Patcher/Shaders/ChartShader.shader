    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fragColor =  lerp(fragColor, _LineColor, segment(i.uv, float2(i.uv.x , _StepHeight[round(i.uv.x * _NumberOfSamples) - 1]), float2(i.uv.x , _StepHeight[round(i.uv.x * _NumberOfSamples)]), borderTopLine));
                fragColor =  lerp(fragColor, _LineColor, segment(i.uv, float2(i.uv.x , _StepHeight[round(i.uv.x * _NumberOfSamples) + 1]), float2(i.uv.x , _StepHeight[round(i.uv.x * _NumberOfSamples)]), borderTopLine));
                return fragColor;
            }
            ENDCG
