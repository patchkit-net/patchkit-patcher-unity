...
public class UnityDispatcher : MonoBehaviour
{
    ...
    private static void ValidateInstance()
    {
        if (_instance == null)
        {
            throw new InvalidOperationException(
                "Dispatcher has to be initialized before any usage.");
        }
    }
    ...
    private static EventWaitHandle BaseInvoke(
        Action<ManualResetEvent> actionStarter)
    {
        ValidateInstance();
        ...
        else
        {
            ...
            {
                _instance._pendingActions.Enqueue(
                    () => actionStarter(manualResetEvent));
            }
        }
    ...
    {
        return BaseInvoke(
            manualResetEvent => ActionWithEventWaitHandle(
                action,
                manualResetEvent));
    }

    private static void ActionWithEventWaitHandle(
        Action action,
        ManualResetEvent
            manualResetEvent)
    {
    ...
    {
        return BaseInvoke(
            manualResetEvent => _instance.StartCoroutine(
                CoroutineWithEventWaitHandle(
                    coroutine,
                    manualResetEvent)));
    }

    private static IEnumerator CoroutineWithEventWaitHandle(
