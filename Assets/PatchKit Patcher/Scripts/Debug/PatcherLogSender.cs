...
namespace PatchKit.Unity.Patcher.Debug
{
    public class PatcherLogSender : MonoBehaviour
    {
        ...
        private void Update()
        {
            lock (_lock)
            {
                if (!_isLogBeingSent)
                {
                    ...
                    if (_logToBeSent.HasValue && _sendLogKind.HasValue)
                    {
                        _isLogBeingSent = true;
                        StartCoroutine(SendLogFile(_logToBeSent.Value));
                        _logToBeSent = null;
                        _sendLogKind = null;
                    }
                }
        ...
        {
            if (_isLogBeingSent || _logToBeSent.HasValue)
            {
        ...
        private IEnumerator SendLogFile(Log log)
        {
            ...
            DebugLogger.Log("Registering event on Sentry");

            _ravenClient.Capture(new SentryEvent(new SentryMessage(log.Message))
            {
                Level = log.GetErrorLevel(),
                Extra = _guid.ToString()
            });
