...
namespace PatchKit.Unity.Patcher.Data.Remote.Downloaders
{
    ...
    /// </summary>
    class ChunkedHttpDownloader : IDisposable
    {
        ...
        public ChunkedHttpDownloader(string[] urls, long fileSize, string destinationFilePath,
        ...
        public bool Start(CustomProgressReporter<DownloadProgress> progressReporter, CancellationToken cancellationToken)
        {
            if (_started)
            {
                throw new BadStateException("Cannot start the same ChunkedHttpDownloader twice.");
            }
            _started = true;
